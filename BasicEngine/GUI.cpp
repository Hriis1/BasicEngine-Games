...
namespace BasicEngine {
	...
	{
		//Check if the renderer and system have already been inited
		if (_renderer == nullptr)
		{
			...
			CEGUI::ScriptModule::setDefaultResourceGroup("lua_scrips");
		}
	...
	void GUI::onSDLEvent(SDL_Event& evnt)
	{
		...
		case SDL_TEXTINPUT:
            codePoint = 0;
            for (size_t i = 0; evnt.text.text[i] != '\0'; i++)
            {
	...
	void GUI::setMouseCursor(const std::string& imgFile)
	{
		_context->getMouseCursor().setDefaultImage(imgFile);
	}
